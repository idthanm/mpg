...
class OffPolicyMBLearner(object):
    ...
    def compute_td_errors(self):
        ...
        nextnonterminal = 1 - self.batch_data['batch_dones']

        batch_values = self.policy_with_value.compute_Q(processed_batch_obs, self.batch_data['batch_actions']).numpy()[:, 0]
        act_tp1, _ = self.policy_with_value.compute_action(processed_batch_obs_tp1)
        batch_values_tp1 = self.policy_with_value.compute_Q_target(processed_batch_obs_tp1, act_tp1.numpy()).numpy()[:, 0]
        td_errors = processed_batch_rewards + self.args.gamma * batch_values_tp1 * nextnonterminal.astype(np.float32)\
                    - batch_values
        return td_errors
